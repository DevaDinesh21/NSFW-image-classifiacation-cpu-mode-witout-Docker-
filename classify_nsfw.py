...
def caffe_preprocess_and_compute(pimg, caffe_transformer=None, caffe_net=None, output_layers=None):
    ...
    if caffe_net is not None:
        ...
        image_sc = caffe.io.load_image(BytesIO(img_data_rs))

        H, W, _ = image_sc.shape
        _, _, h, w = caffe_net.blobs["data"].data.shape
        h_off = max((H - h) // 2, 0)
        w_off = max((W - w) // 2, 0)
        crop = image_sc[h_off:h_off + h, w_off:w_off + w, :]
...
def get_score(file, model=default_model, pretrained_model=default_pretrained_model):
    ...
    if file.find('http') != -1:
        ...
        tmp_filename = "temp/" + random_text + "out.jpg"
    ...
    # Classify.
    scores = caffe_preprocess_and_compute(image_score, caffe_transformer=caffe_transformer, caffe_net=nsfw_net,
                                          output_layers=["prob"])
